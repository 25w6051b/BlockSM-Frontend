...
const guidanceTexts = {
    ...
    guidance7: '<span class="center-target">[ ～という作用が起こる ]</span> <img src="img/effectDefinition3.png"/><img src="img/effectDefinition.png"/> 状態が<strong>切り替わる</strong>ときに実行される<strong>処理</strong>を決めるためのブロックです。（同じ状態に戻る場合も、状態が切り替わったものとして扱われます。）\n『エフェクト』ブロックから実行したい処理を選び、状態が変わるタイミングで行いたい<span>処理</span>をこの中に繋いで使いましょう。\n処理には、変数の値を<strong>設定</strong>したり<strong>増減</strong>させたりすることもできます。',
    guidance8: '<span class="center-target">[ 変数 ]</span><img src="img/variableDefinition.png"/> 変数の値を<strong>設定・増減</strong>したり<strong>条件</strong>を決めるためのブロックです。\n<strong>『条件』ブロック</strong>と組み合わせて、変数の値をもとに状態を切り替える<strong>条件</strong>を決めたり、<strong>『作用』ブロック</strong>と組み合わせて、状態が切り替わるタイミングで値を<strong>設定・増減</strong>する処理に使えます。',
}
